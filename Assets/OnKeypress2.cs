...
public class OnKeypress2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Keypad0)) componentToHide.enabled = true;
        ...
        if (Input.GetKeyUp(KeyCode.Keypad0))
        {
            ...
            Physics2D.IgnoreCollision(curr_snowball.GetComponent<Collider2D>(), GameObject.FindGameObjectWithTag("Player2").GetComponent<Collider2D>());
        ...
        //test.position = new Vector3(-1f*newX , newY , 0);

        lineRenderer.SetPosition(1, new Vector3(-1f * newX, newY, 0));
    }
}